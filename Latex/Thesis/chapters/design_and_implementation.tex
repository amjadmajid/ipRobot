...
The software implementation that allows the robot to preform controlled movements consists of 700 lines of MSP430 specific C code and is divided in three parts, the main program, a control loop and motor control.
...
A timer is enabled running at a frequency of a 100\,Hz, used to periodically call an interrupt service routine (ISR) and execute the control loop.
If the ISR is triggered, first a yaw-rate sample is requested from the gyroscope.
Then based on the movement that is currently executed, an evaluation is done to determine if the movement target is reached.
If this is not the case then the yaw-rate supplied to the PID controller to produce an output, which is used to update the motor values.
The output is subtracted from the left motor value and added to the right motor value, in order to keep the average speed roughly the same.
...
On the other hand, curved movements use an angle as the movement target.
...
The H-bridge is configured in IN/IN mode, i.e. each set of two IO-ports directly controls a motor where one of the ports is enabled for the motor to rotate forwards and only the other port is enabled to make to motor rotate backwards.
...
The transiently powered robot can make one movement or a series of movements is likely to require multiple power cycles to complete.
To be able to finish a movement and not reset i.e. redo the same movement, a simple checkpointing method is used to keep the program state across power cycles.
A persistent counter registers the progress in the set of movements.
Each control loop iteration the persistent variable that captures the progress towards the movement target is updated, and depending on the movement can be a required time or angle.
The right and left motor speed tuned by the PID controller are not saved and restored after a power interrupt.
Due to the startup phase of the motors, previously tuned motor speeds are not valid and cannot be used after a power interrupt. 
