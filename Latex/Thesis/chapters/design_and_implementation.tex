...
In this chapter the hardware design of the transiently-powered robot will be explained in Section \ref{sec:dai_hardware_design}.
...
Finally, Section \ref{sec:dai_software_implementation} will explain the software implementation that enables the robot to execute a movement despite regular power interrupts. 
...
In Section \ref{sec:pre_locomotion_selection} the 25:1 sub-micro plastic planetary gearmotor from Precision Microdrives~\cite{gearmotor_206-110_2017} is chosen to provide locomotion for the robot.
...
By making the assumption that the robot will only travel on a flat surface, the steady state speed is considered ``constant".
%given a certain PWM duty cycle.
