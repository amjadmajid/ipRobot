...
Additionally, the angular velocity of the motor will decrease because the torque is inversely proportional to the angular velocity of the motor.

\subsubsection{Start-up Current Peak}
The DC motors used for robots are normally powered directly from the battery as linear or switch-mode power regulators are not able to supply the high start-up currents.
In the worst case the start-up current peak can be equal to the stall current of the motor.
...
To evaluate the magnitude of the current peak, the free running current profile of the selected 206-11 DC motor from Precision Microdrives~\cite{gearmotor_206-110_2017} is recorded using a Monsoon Power Monitor~\cite{monsoon_powermonitor_2017}.
