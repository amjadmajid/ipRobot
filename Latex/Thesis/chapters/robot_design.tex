...
This makes open loop movement using just a calibrated motor values possible but time consuming and any little disturbance will trow the robot off course.
However, using the angular velocity around the z-axis which can be obtained from the gyroscope, the robot can use closed loop feedback to keep the angular velocity close to zero.

Closed loop use the angular velocity in the z direction
