...
Currently only the microcontroller is being utilized, but a temperature and triaxial accelerometer are included as well. %and backscatter communication.
...
Therefore the overhead of minimized while no interrupt service routine is required and the ports are toggled directly.
...
%TODO TELL WHY 100HZ (get the required precision after intergrating to angle)
...
%TODO -Write about minimum output to motors (add min number), adding the minimum made it much more responsive and quicker to tune.
...
%- not fully true? However, the oscillation is bounded because the motor speed is also bounded by a maximum.
