...
On edge cases were energy is depleted while the input is read, the validity of the input can be questioned.
When the device wakes up from a depleted state, in the case the input was time depended it should discarded.

However using a micro-controller to control another device operating under an intermittent power supply provides new challenges.
With intermittent output control it can be doubtful if an output operation can be redone, while the progress when dying is uncertain.
The assumption is made that in general the output is on for long enough to complete the operation.
Therefore only two point's in time can be certain, before and after doing the output operation.
...
% Differential drive robots are known to be problematic to  accurate odometry.
...
% Update step
The update step is used to compute the effective sample size as well.
